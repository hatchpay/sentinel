...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, hatchd):
        golist = hatchd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_hatchd(hatchd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_hatchd(self, hatchd, rec):
        import decimal
        import hatchlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from hatchd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from hatchd, with every run
        ...
        # get/create, then sync payment amounts, etc. from hatchd - Hatchd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(hatchd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from hatchd! %s" % e)
            govobj.vote_delete(hatchd)
            return (govobj, None)
    ...
    def vote_delete(self, hatchd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(hatchd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, hatchd, signal, outcome):
        import hatchlib
        ...
        output = hatchd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = hatchlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(hatchd, signal)

    def sync_network_vote(self, hatchd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = hatchd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = HATCHD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 hatch addr, non-multisig
            if not hatchlib.is_valid_hatch_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Hatch address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Hatch Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (hatchlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = HATCHD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not hatchlib.is_valid_hatch_address(addr, config.network):
    ...
    def hash(self):
        import hatchlib
        return hatchlib.hashit(self.serialise())
