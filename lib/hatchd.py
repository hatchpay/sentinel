...
class HatchDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some hatchd methods
    ...
    @classmethod
    def from_hatch_conf(self, hatch_dot_conf):
        from hatch_config import HatchConfig
        config_text = HatchConfig.slurp_config_file(hatch_dot_conf)
        creds = HatchConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = hatchlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = hatchlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = hatchlib.blocks_to_seconds(diff)
