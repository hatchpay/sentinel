...
def is_valid_hatch_address(address, network='mainnet'):
    ...
    hatch_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Hatch address
        return False

    if (address_version != hatch_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('hatch.conf')
